...
class SearchController < ApplicationController
  ...
  def member_search_params
    ...
    search_params[:birthday_lteq] = Date.current.prev_year(search_params[:birthday_lteq].to_i) if search_params[:birthday_lteq].present?
    search_params[:birthday_gteq] = Date.current.prev_year(search_params[:birthday_gteq].to_i + 1).tomorrow if search_params[:birthday_gteq].present?
    search_params
  end
